...
void IIR::init()
{
    ...
    buff_x_ptr=0;
    buff_y_ptr=0;
    std::fill(buff_x.begin(), buff_x.end(), 0.0);
    std::fill(buff_y.begin(), buff_y.end(), 0.0);
}

double  IIR::update(double sig)
{

    if((int)buff_x.size() != (int)b.size())
    {
        buff_x.resize(b.size());
    }
    if((int)buff_y.size() != (int)a.size()-1)
    {
        buff_y.resize(a.size()-1);
    }
    if(buff_x_ptr>=(int)buff_x.size())buff_x_ptr=0;
    if(buff_y_ptr>=(int)buff_y.size())buff_y_ptr=0;

    ASSERTCH(buff_x,buff_x_ptr);
    buff_x[buff_x_ptr]=sig;
    buff_x_ptr++;buff_x_ptr%=(int)buff_x.size();
    ...
    for(int i=(int)a.size()-1;i>=1;i--)
    {
        ...
        buff_y_ptr++;buff_y_ptr%=(int)buff_y.size();
    }
...
std::vector<JFFT::cpx_type> QJHilbertFilter::getKernel()
{
    return kernel;
}
